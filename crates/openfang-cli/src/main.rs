...
fn config_log_level() -> String {
    let config_path = if let Ok(home) = std::env::var("IA_BRO_HOME") {
        std::path::PathBuf::from(home).join("config.toml")
    } else if let Ok(home) = std::env::var("OPENFANG_HOME") {
        std::path::PathBuf::from(home).join("config.toml")
    } else {
        dirs::home_dir()
...
/// Get the IA,BRO! home directory, respecting IA_BRO_HOME or OPENFANG_HOME env var.
fn cli_openfang_home() -> std::path::PathBuf {
    if let Ok(home) = std::env::var("IA_BRO_HOME") {
