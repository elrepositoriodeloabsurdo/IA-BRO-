/**
 * IA,BRO! i18n (Internationalization) Module
 * 
 * Provides runtime language switching for the IA,BRO! dashboard UI.
...
(function() {
  ...
  function applyTranslations() {
    ...
    if (metaDesc) {
      const desc = t('app.description', { name: 'IA,BRO!' });
  ...
  async function setLanguage(lang, persist = true) {
    ...
    // Save preference
    if (persist) {
      localStorage.setItem('iabro_language', lang);
    }
  ...
  async function init() {
    ...
    let lang = localStorage.getItem('iabro_language');
