// Vibecodingchile IA,BRO! Settings Page — Provider Hub, Model Catalog, Config, Tools + Security, Network, Migration tabs
'use strict';
  ...
  return {
    ...
    configurableFeatures: [
      ...
      },
      {
        ...
        configHint: 'Set api_key in ~/.iabro/config.toml for remote access. Empty = localhost only.',
        valueKey: 'auth'
      }
    ],
    ...
    async loadSysInfo() {
      try {
        var ver = await IABroAPI.get('/api/version');
        var status = await IABroAPI.get('/api/status');
        this.sysInfo = {
          version: ver.version || '-',
    ...
    async loadUsage() {
      try {
        var data = await IABroAPI.get('/api/usage');
    ...
    async loadTools() {
      try {
        var data = await IABroAPI.get('/api/tools');
    ...
    async loadConfig() {
      try {
        this.config = await IABroAPI.get('/api/config');
      } catch(e) { this.config = {}; }
    },

    async loadProviders() {
      try {
        var data = await IABroAPI.get('/api/providers');
    ...
    async loadModels() {
      try {
        var data = await IABroAPI.get('/api/models');
    ...
    async addCustomModel() {
      ...
      try {
        await IABroAPI.post('/api/models/custom', {
          id: id,
    ...
    async deleteCustomModel(modelId) {
      ...
      try {
        await IABroAPI.del('/api/models/custom/' + encodeURIComponent(modelId));
        IABroToast.success('Model deleted');
        await this.loadModels();
      } catch(e) {
        IABroToast.error('Failed to delete: ' + (e.message || 'Unknown error'));
      }
    },
    ...
    async loadConfigSchema() {
      try {
        var results = await Promise.all([
          IABroAPI.get('/api/config/schema').catch(function() { return {}; }),
          IABroAPI.get('/api/config')
        ]);
    ...
    async saveConfigField(section, field, value) {
      ...
      try {
        await IABroAPI.post('/api/config/set', { path: path, value: value });
        this.configDirty[key] = false;
        IABroToast.success('Saved ' + field);
      } catch(e) {
        IABroToast.error('Failed to save: ' + e.message);
      }
    ...
    async saveProviderKey(provider) {
      ...
      if (!key || !key.trim()) { IABroToast.error('Please enter an API key'); return; }
      try {
        var resp = await IABroAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key.trim() });
        if (resp && resp.switched_default) {
          IABroToast.warning(resp.message || 'Default provider was switched to ' + provider.display_name);
        } else {
          IABroToast.success('API key saved for ' + provider.display_name);
        }
        this.providerKeyInputs[provider.id] = '';
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        IABroToast.error('Failed to save key: ' + e.message);
      }
    },

    async removeProviderKey(provider) {
      try {
        await IABroAPI.del('/api/providers/' + encodeURIComponent(provider.id) + '/key');
        IABroToast.success('API key removed for ' + provider.display_name);
        await this.loadProviders();
        await this.loadModels();
      } catch(e) {
        IABroToast.error('Failed to remove key: ' + e.message);
      }
    },
    ...
    async startCopilotOAuth() {
      ...
      try {
        var resp = await IABroAPI.post('/api/providers/github-copilot/oauth/start', {});
        ...
        IABroToast.error('Failed to start Copilot login: ' + e.message);
    ...
    pollCopilotOAuth() {
      ...
      setTimeout(async function() {
        ...
        try {
          var resp = await IABroAPI.get('/api/providers/github-copilot/oauth/poll/' + self.copilotOAuth.pollId);
          if (resp.status === 'complete') {
            IABroToast.success('GitHub Copilot authenticated successfully!');
            ...
            IABroToast.error('Device code expired. Please try again.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else if (resp.status === 'denied') {
            IABroToast.error('Access denied by user.');
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          } else {
            IABroToast.error('OAuth error: ' + (resp.error || resp.status));
            self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
          }
        } catch(e) {
          IABroToast.error('Poll error: ' + e.message);
          self.copilotOAuth = { polling: false, userCode: '', verificationUri: '', pollId: '', interval: 5 };
        }
    ...
    async testProvider(provider) {
      ...
      try {
        var result = await IABroAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/test', {});
        this.providerTestResults[provider.id] = result;
        if (result.status === 'ok') {
          IABroToast.success(provider.display_name + ' connected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          IABroToast.error(provider.display_name + ': ' + (result.error || 'Connection failed'));
        }
      } catch(e) {
        this.providerTestResults[provider.id] = { status: 'error', error: e.message };
        IABroToast.error('Test failed: ' + e.message);
      }
    ...
    async saveProviderUrl(provider) {
      ...
      if (!url || !url.trim()) { IABroToast.error('Please enter a base URL'); return; }
      url = url.trim();
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        IABroToast.error('URL must start with http:// or https://'); return;
      }
      this.providerUrlSaving[provider.id] = true;
      try {
        var result = await IABroAPI.put('/api/providers/' + encodeURIComponent(provider.id) + '/url', { base_url: url });
        if (result.reachable) {
          IABroToast.success(provider.display_name + ' URL saved &mdash; reachable (' + (result.latency_ms || '?') + 'ms)');
        } else {
          IABroToast.warning(provider.display_name + ' URL saved but not reachable');
        }
        await this.loadProviders();
      } catch(e) {
        IABroToast.error('Failed to save URL: ' + e.message);
      }
    ...
    async addCustomProvider() {
      ...
      if (!name) { IABroToast.error('Please enter a provider name'); return; }
      var url = this.customProviderUrl.trim();
      if (!url) { IABroToast.error('Please enter a base URL'); return; }
      if (url.indexOf('http://') !== 0 && url.indexOf('https://') !== 0) {
        IABroToast.error('URL must start with http:// or https://'); return;
      }
      ...
      try {
        var result = await IABroAPI.put('/api/providers/' + encodeURIComponent(name) + '/url', { base_url: url });
        if (this.customProviderKey.trim()) {
          await IABroAPI.post('/api/providers/' + encodeURIComponent(name) + '/key', { key: this.customProviderKey.trim() });
        }
        this.customProviderName = '';
        this.customProviderUrl = '';
        this.customProviderKey = '';
        this.customProviderStatus = '';
        IABroToast.success('Provider "' + name + '" added' + (result.reachable ? ' (reachable)' : ' (not reachable yet)'));
        await this.loadProviders();
      } catch(e) {
        this.customProviderStatus = 'Error: ' + (e.message || 'Failed');
        IABroToast.error('Failed to add provider: ' + e.message);
      }
    ...
    async loadSecurity() {
      ...
      try {
        this.securityData = await IABroAPI.get('/api/security');
      } catch(e) {
    ...
    async verifyAuditChain() {
      ...
      try {
        var res = await IABroAPI.get('/api/audit/verify');
    ...
    async loadPeers() {
      ...
      this.peersLoadError = '';
      try {
        var data = await IABroAPI.get('/api/peers');
    ...
    async autoDetect() {
      this.detecting = true;
      try {
        var data = await IABroAPI.get('/api/migrate/detect');
    ...
    async scanPath() {
      ...
      this.scanning = true;
      try {
        var data = await IABroAPI.post('/api/migrate/scan', { path: this.sourcePath });
        if (data.error) {
          IABroToast.error('Scan error: ' + data.error);
          this.scanning = false;
          return;
        }
        ...
        IABroToast.error('Scan failed: ' + e.message);
      }
      this.scanning = false;
    },
    ...
    async runMigration(dryRun) {
      ...
      try {
        ...
        if (!target) target = '';
        var data = await IABroAPI.post('/api/migrate', {
          source: 'openclaw',
