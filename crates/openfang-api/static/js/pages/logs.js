// Vibecodingchile IA,BRO! Logs Page — Real-time log viewer (SSE streaming + polling fallback) + Audit Trail tab
'use strict';
...
function logsPage() {
  return {
    ...
    async fetchLogs() {
      ...
      try {
        var data = await IABroAPI.get('/api/audit/recent?n=200');
    ...
    exportLogs: function() {
      ...
      a.href = url;
      a.download = 'iabro-logs-' + new Date().toISOString().slice(0, 10) + '.txt';
      a.click();
    ...
    async verifyChain() {
      try {
        var data = await IABroAPI.get('/api/audit/verify');
        this.chainValid = data.valid === true;
        if (this.chainValid) {
          IABroToast.success('Audit chain verified — ' + (data.entries || 0) + ' entries valid');
        } else {
          IABroToast.error('Audit chain broken!');
        }
      } catch(e) {
        this.chainValid = false;
        IABroToast.error('Chain verification failed: ' + e.message);
      }
    },
