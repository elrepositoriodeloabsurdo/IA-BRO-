// Vibecodingchile IA,BRO! Channels Page — OpenClaw-style setup UX with QR code support
'use strict';
...
function channelsPage() {
  return {
    ...
    async startQR() {
      ...
      try {
        var result = await IABroAPI.post('/api/channels/whatsapp/qr/start', {});
        ...
        if (this.qr.connected) {
          IABroToast.success('WhatsApp connected!');
    ...
    pollQR() {
      ...
      this.qrPollTimer = setInterval(async function() {
        try {
          var result = await IABroAPI.get('/api/channels/whatsapp/qr/status?session_id=' + encodeURIComponent(self.qr.sessionId));
          if (result.connected) {
            clearInterval(self.qrPollTimer);
            self.qrPollTimer = null;
            self.qr.connected = true;
            self.qr.message = result.message || 'Connected!';
            IABroToast.success('WhatsApp linked successfully!');
    ...
    async saveChannel() {
      ...
      try {
        await IABroAPI.post('/api/channels/' + name + '/configure', {
          fields: this.formValues
        });
        this.setupStep = 2;
        // Auto-test after save
        try {
          var testResult = await IABroAPI.post('/api/channels/' + name + '/test', {});
          if (testResult.status === 'ok') {
            this.testPassed = true;
            this.setupStep = 3;
            IABroToast.success(this.setupModal.display_name + ' activated!');
          } else {
            IABroToast.success(this.setupModal.display_name + ' saved. ' + (testResult.message || ''));
          }
        } catch(te) {
          IABroToast.success(this.setupModal.display_name + ' saved. Test to verify connection.');
        }
        await this.refreshStatus();
      } catch(e) {
        IABroToast.error('Failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async removeChannel() {
      ...
      var self = this;
      IABroToast.confirm('Remove Channel', 'Remove ' + displayName + ' configuration? This will deactivate the channel.', async function() {
        try {
          await IABroAPI.delete('/api/channels/' + name + '/configure');
          IABroToast.success(displayName + ' removed and deactivated.');
          await self.refreshStatus();
    ...
    async testChannel() {
      ...
      try {
        var result = await IABroAPI.post('/api/channels/' + name + '/test', {});
        if (result.status === 'ok') {
          this.testPassed = true;
          this.setupStep = 3;
          IABroToast.success(result.message);
        } else {
          IABroToast.error(result.message);
        }
      } catch(e) {
        IABroToast.error('Test failed: ' + (e.message || 'Unknown error'));
      }
    ...
    async copyConfig(ch) {
      ...
      try {
        ...
        IABroToast.success('Copied to clipboard');
      } catch(e) {
        IABroToast.error('Copy failed');
      }
    },
