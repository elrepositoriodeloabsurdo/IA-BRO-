// Vibecodingchile IA,BRO! Sessions Page — Session listing + Memory tab
'use strict';
...
function sessionsPage() {
  return {
    ...
    async loadSessions() {
      ...
      this.loadError = '';
      try {
        var data = await IABroAPI.get('/api/sessions');
    ...
    deleteSession(sessionId) {
      ...
      IABroToast.confirm(
        ...
        async function() {
          try {
            await IABroAPI.del('/api/sessions/' + sessionId);
            self.sessions = self.sessions.filter(function(s) { return s.session_id !== sessionId; });
            IABroToast.success('Session deleted');
          } catch(e) {
            IABroToast.error('Failed to delete session: ' + e.message);
          }
        }
      );
    },
    ...
    async loadKv() {
      ...
      this.memLoadError = '';
      try {
        var data = await IABroAPI.get('/api/memory/agents/' + this.memAgentId + '/kv');
    ...
    async addKey() {
      ...
      try {
        await IABroAPI.put('/api/memory/agents/' + this.memAgentId + '/kv/' + encodeURIComponent(this.newKey), { value: value });
        this.showAdd = false;
        IABroToast.success('Key "' + this.newKey + '" saved');
        this.newKey = '';
        this.newValue = '""';
        await this.loadKv();
      } catch(e) {
    ...
    deleteKey(key) {
      ...
      IABroToast.confirm(
        ...
        async function() {
          try {
            await IABroAPI.del('/api/memory/agents/' + self.memAgentId + '/kv/' + encodeURIComponent(key));
            IABroToast.success('Key "' + key + '" deleted');
            await self.loadKv();
          } catch(e) {
            IABroToast.error('Failed to delete key: ' + e.message);
          }
        }
      );
    },
    ...
    async saveEdit() {
      ...
      try {
        await IABroAPI.put('/api/memory/agents/' + this.memAgentId + '/kv/' + encodeURIComponent(this.editingKey), { value: value });
        IABroToast.success('Key "' + this.editingKey + '" updated');
        this.editingKey = null;
        this.editingValue = '';
        await this.loadKv();
      } catch(e) {
