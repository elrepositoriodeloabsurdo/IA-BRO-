// Vibecodingchile IA,BRO! Agents Page — Multi-step spawn wizard, detail view with tabs, file editor, personality presets
'use strict';
...
function agentsPage() {
  return {
    ...
    async loadSpawnProfiles() {
      ...
      try {
        var data = await IABroAPI.get('/api/profiles');
    ...
    async loadTemplates() {
      ...
      try {
        var results = await Promise.all([
          IABroAPI.get('/api/templates'),
          IABroAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    async showDetail(agent) {
      ...
      var detail = agent;
      try {
        var full = await IABroAPI.get('/api/agents/' + agent.id);
    ...
    killAgent(agent) {
      var self = this;
      IABroToast.confirm('Stop Agent', 'Stop agent "' + agent.name + '"? The agent will be shut down.', async function() {
        try {
          await IABroAPI.del('/api/agents/' + agent.id);
          IABroToast.success('Agent "' + agent.name + '" stopped');
          self.showDetailModal = false;
          await Alpine.store('app').refreshAgents();
        } catch(e) {
          IABroToast.error('Failed to stop agent: ' + e.message);
        }
      });
    },

    // Issue #1163: uninstall an agent (kill + remove ~/.iabro/agents/<name>/).
    uninstallAgent(agent) {
      ...
      IABroToast.confirm(
        ...
        async function() {
          try {
            var res = await IABroAPI.del('/api/agents/' + agent.id + '/uninstall');
            var msg = 'Agent "' + agent.name + '" uninstalled';
            if (res && res.dir_removed === false) {
              msg += ' (no on-disk files found)';
            }
            IABroToast.success(msg);
            self.showDetailModal = false;
            await Alpine.store('app').refreshAgents();
          } catch(e) {
            IABroToast.error('Failed to uninstall agent: ' + e.message);
          }
        }
      );
    },
    ...
    killAllAgents() {
      ...
      IABroToast.confirm('Stop All Agents', 'Stop ' + list.length + ' agent(s)? All agents will be shut down.', async function() {
        var errors = [];
        for (var i = 0; i < list.length; i++) {
          try {
            await IABroAPI.del('/api/agents/' + list[i].id);
          } catch(e) { errors.push(list[i].name + ': ' + e.message); }
        }
        await Alpine.store('app').refreshAgents();
        if (errors.length) {
          IABroToast.error('Some agents failed to stop: ' + errors.join(', '));
        } else {
          IABroToast.success(list.length + ' agent(s) stopped');
        }
      });
    },
    ...
    async openSpawnWizard() {
      ...
      try {
        var results = await Promise.all([
          IABroAPI.get('/api/status').catch(function() { return {}; }),
          IABroAPI.get('/api/providers').catch(function() { return { providers: [] }; })
        ]);
    ...
    nextStep() {
      if (this.spawnStep === 1 && !this.spawnForm.name.trim()) {
        IABroToast.warn('Please enter an agent name');
        return;
      }
    ...
    async setMode(agent, mode) {
      try {
        await IABroAPI.put('/api/agents/' + agent.id + '/mode', { mode: mode });
        agent.mode = mode;
        IABroToast.success('Mode set to ' + mode);
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        IABroToast.error('Failed to set mode: ' + e.message);
      }
    },
    ...
    async spawnAgent() {
      ...
      if (!toml.trim()) {
        this.spawning = false;
        IABroToast.warn('Manifest is empty \u2014 enter agent config first');
        return;
      }

      try {
        var res = await IABroAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          ...
          if (Object.keys(patchBody).length) {
            IABroAPI.patch('/api/agents/' + res.agent_id + '/config', patchBody).catch(function(e) { console.warn('Post-spawn config patch failed:', e.message); });
          }
          if (this.soulContent.trim()) {
            IABroAPI.put('/api/agents/' + res.agent_id + '/files/SOUL.md', { content: '# Soul\n' + this.soulContent }).catch(function(e) { console.warn('SOUL.md write failed:', e.message); });
          }
          ...
          this.spawnToml = '';
          this.spawnStep = 1;
          IABroToast.success('Agent "' + (res.name || 'new') + '" spawned');
          await Alpine.store('app').refreshAgents();
          this.chatWithAgent({ id: res.agent_id, name: res.name, model_provider: '?', model_name: '?' });
        } else {
          IABroToast.error('Spawn failed: ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        IABroToast.error('Failed to spawn agent: ' + e.message);
      }
      this.spawning = false;
    },
    ...
    async loadAgentFiles() {
      ...
      try {
        var data = await IABroAPI.get('/api/agents/' + this.detailAgent.id + '/files');
        this.agentFiles = data.files || [];
      } catch(e) {
        this.agentFiles = [];
        IABroToast.error('Failed to load files: ' + e.message);
      }
    ...
    async openFile(file) {
      ...
      try {
        var data = await IABroAPI.get('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(file.name));
        this.editingFile = file.name;
        this.fileContent = data.content || '';
      } catch(e) {
        IABroToast.error('Failed to read file: ' + e.message);
      }
    },
    ...
    async saveFile() {
      ...
      try {
        await IABroAPI.put('/api/agents/' + this.detailAgent.id + '/files/' + encodeURIComponent(this.editingFile), { content: this.fileContent });
        IABroToast.success(this.editingFile + ' saved');
        await this.loadAgentFiles();
      } catch(e) {
        IABroToast.error('Failed to save file: ' + e.message);
      }
    ...
    async saveConfig() {
      ...
      try {
        await IABroAPI.patch('/api/agents/' + this.detailAgent.id + '/config', this.configForm);
        IABroToast.success('Config updated');
        await Alpine.store('app').refreshAgents();
      } catch(e) {
        IABroToast.error('Failed to save config: ' + e.message);
      }
    ...
    async cloneAgent(agent) {
      ...
      try {
        var res = await IABroAPI.post('/api/agents/' + agent.id + '/clone', { new_name: newName });
        if (res.agent_id) {
          IABroToast.success('Cloned as "' + res.name + '"');
          await Alpine.store('app').refreshAgents();
          this.showDetailModal = false;
        }
      } catch(e) {
        IABroToast.error('Clone failed: ' + e.message);
      }
    },
    ...
    async spawnFromTemplate(template) {
      try {
        ...
        if (!manifestToml) {
          ...
          var data = await IABroAPI.get('/api/templates/' + encodeURIComponent(template.name));
          manifestToml = data.manifest_toml;
        }
        if (manifestToml) {
          var res = await IABroAPI.post('/api/agents', { manifest_toml: manifestToml });
          if (res.agent_id) {
            IABroToast.success('Agent "' + (res.name || template.name) + '" spawned from template');
            await Alpine.store('app').refreshAgents();
            this.chatWithAgent({ id: res.agent_id, name: res.name || template.name, model_provider: '?', model_name: '?' });
          }
        }
      } catch(e) {
        IABroToast.error('Failed to spawn from template: ' + e.message);
      }
    },
    ...
    async clearHistory(agent) {
      var self = this;
      IABroToast.confirm('Clear History', 'Clear all conversation history for "' + agent.name + '"? This cannot be undone.', async function() {
        try {
          await IABroAPI.del('/api/agents/' + agent.id + '/history');
          IABroToast.success('History cleared for "' + agent.name + '"');
        } catch(e) {
          IABroToast.error('Failed to clear history: ' + e.message);
        }
      });
    },
    ...
    async changeModel() {
      ...
      try {
        var resp = await IABroAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: this.newModelValue.trim() });
        var providerInfo = (resp && resp.provider) ? ' (provider: ' + resp.provider + ')' : '';
        IABroToast.success('Model changed' + providerInfo + ' (memory reset)');
        ...
        IABroToast.error('Failed to change model: ' + e.message);
      }
    ...
    async changeProvider() {
      ...
      try {
        ...
        var resp = await IABroAPI.put('/api/agents/' + this.detailAgent.id + '/model', { model: combined });
        IABroToast.success('Provider changed to ' + (resp && resp.provider ? resp.provider : this.newProviderValue.trim()));
        ...
        IABroToast.error('Failed to change provider: ' + e.message);
      }
      this.modelSaving = false;
    },
    ...
    async addFallback() {
      ...
      try {
        await IABroAPI.patch('/api/agents/' + this.detailAgent.id + '/config', {
          fallback_models: this.detailAgent._fallbacks
        });
        IABroToast.success('Fallback added: ' + provider + '/' + model);
      } catch(e) {
        IABroToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async removeFallback(idx) {
      ...
      try {
        ...
        IABroToast.success('Fallback removed');
      } catch(e) {
        IABroToast.error('Failed to save fallbacks: ' + e.message);
    ...
    async loadToolFilters() {
      ...
      try {
        this.toolFilters = await IABroAPI.get('/api/agents/' + this.detailAgent.id + '/tools');
      } catch(e) {
    ...
    async saveToolFilters() {
      if (!this.detailAgent) return;
      try {
        await IABroAPI.put('/api/agents/' + this.detailAgent.id + '/tools', this.toolFilters);
      } catch(e) {
        IABroToast.error('Failed to update tool filters: ' + e.message);
      }
    },
    ...
    async spawnBuiltin(t) {
      ...
      try {
        var res = await IABroAPI.post('/api/agents', { manifest_toml: toml });
        if (res.agent_id) {
          IABroToast.success('Agent "' + t.name + '" spawned');
          await Alpine.store('app').refreshAgents();
