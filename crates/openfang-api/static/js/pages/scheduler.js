// Vibecodingchile IA,BRO! Scheduler Page — Cron job management + event triggers unified view
'use strict';
...
function schedulerPage() {
  return {
    ...
    async loadChannelTypes() {
      try {
        var data = await IABroAPI.get('/api/channels');
    ...
    async loadJobs() {
      var data = await IABroAPI.get('/api/cron/jobs');
      var raw = data.jobs || [];
    ...
    async loadTriggers() {
      ...
      this.trigLoadError = '';
      try {
        var data = await IABroAPI.get('/api/triggers');
    ...
    async createJob() {
      if (!this.newJob.name.trim()) {
        IABroToast.warn('Please enter a job name');
        return;
      }
      if (!this.newJob.cron.trim()) {
        IABroToast.warn('Please enter a cron expression');
        return;
      }
      this.creating = true;
      try {
        ...
        await IABroAPI.post('/api/cron/jobs', body);
        this.showCreateForm = false;
        this.newJob = { name: '', cron: '', agent_id: '', message: '', enabled: true, delivery_targets: [] };
        IABroToast.success('Schedule "' + jobName + '" created');
        await this.loadJobs();
      } catch(e) {
        IABroToast.error('Failed to create schedule: ' + (e.message || e));
      }
      this.creating = false;
    },
    ...
    async toggleJob(job) {
      try {
        ...
        await IABroAPI.put('/api/cron/jobs/' + job.id + '/enable', { enabled: newState });
        job.enabled = newState;
        IABroToast.success('Schedule ' + (newState ? 'enabled' : 'paused'));
      } catch(e) {
        IABroToast.error('Failed to toggle schedule: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      IABroToast.confirm('Delete Schedule', 'Delete "' + jobName + '"? This cannot be undone.', async function() {
        try {
          await IABroAPI.del('/api/cron/jobs/' + job.id);
          self.jobs = self.jobs.filter(function(j) { return j.id !== job.id; });
          IABroToast.success('Schedule "' + jobName + '" deleted');
        } catch(e) {
          IABroToast.error('Failed to delete schedule: ' + (e.message || e));
        }
      });
    },
    ...
    async runNow(job) {
      ...
      try {
        var result = await IABroAPI.post('/api/cron/jobs/' + job.id + '/run', {});
        if (result.status === 'triggered' || result.status === 'completed') {
          IABroToast.success('Job "' + (job.name || 'job') + '" triggered');
          ...
          IABroToast.error('Run failed: ' + (result.error || 'Unknown error'));
        }
      } catch(e) {
        IABroToast.error('Run failed: ' + (e.message || e));
      }
      this.runningJobId = '';
    },
    ...
    async toggleExpand(job) {
      ...
      try {
        var data = await IABroAPI.get('/api/schedules/' + job.id + '/delivery-log');
        this.deliveryLog = {
    ...
    async saveEditTargets() {
      ...
      try {
        ...
        await IABroAPI.put('/api/schedules/' + this.editingTargetsJobId, {
          delivery_targets: clean
        });
        IABroToast.success('Delivery targets updated');
        this.cancelEditTargets();
        await this.loadJobs();
      } catch(e) {
        IABroToast.error('Failed to update targets: ' + (e.message || e));
      }
    ...
    async toggleTrigger(trigger) {
      try {
        ...
        await IABroAPI.put('/api/triggers/' + trigger.id, { enabled: newState });
        trigger.enabled = newState;
        IABroToast.success('Trigger ' + (newState ? 'enabled' : 'disabled'));
      } catch(e) {
        IABroToast.error('Failed to toggle trigger: ' + (e.message || e));
      }
    },

    deleteTrigger(trigger) {
      var self = this;
      IABroToast.confirm('Delete Trigger', 'Delete this trigger? This cannot be undone.', async function() {
        try {
          await IABroAPI.del('/api/triggers/' + trigger.id);
          self.triggers = self.triggers.filter(function(t) { return t.id !== trigger.id; });
          IABroToast.success('Trigger deleted');
        } catch(e) {
          IABroToast.error('Failed to delete trigger: ' + (e.message || e));
        }
      });
    },
