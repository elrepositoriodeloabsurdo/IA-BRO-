// Vibecodingchile IA,BRO! Chat Page — Agent chat with markdown + streaming
'use strict';
  ...
  return {
      ...
      this.$watch('inputText', function(val) {
        ...
        if (modelMatch) {
          ...
          if (!self.modelPickerList.length) {
            IABroAPI.get('/api/models').then(function(data) {
    ...
    toggleModelSwitcher() {
      ...
      IABroAPI.get('/api/models').then(function(data) {
        ...
        IABroToast.error('Failed to load models: ' + e.message);
      });
    },
    ...
    switchModel(model) {
      ...
      var t = typeof window.t === 'function' ? window.t : function(s) { return s; };
      IABroAPI.put('/api/agents/' + this.currentAgent.id + '/model', { model: model.id }).then(function(resp) {
        ...
        IABroToast.success(t('chat.model_switched') + ' ' + (model.display_name || model.id));
        self.showModelSwitcher = false;
        self.modelSwitching = false;
      }).catch(function(e) {
        IABroToast.error(t('chat.model_switch_failed') + ': ' + e.message);
        self.modelSwitching = false;
      });
    },
    ...
    fetchCommands: function() {
      var self = this;
      IABroAPI.get('/api/commands?surface=web').then(function(data) {
      ...
      switch (cmd) {
        ...
        case '/new':
          if (self.currentAgent) {
            IABroAPI.post('/api/agents/' + self.currentAgent.id + '/session/reset', {}).then(function() {
              self.messages = [];
              IABroToast.success('Session reset');
            }).catch(function(e) { IABroToast.error('Reset failed: ' + e.message); });
          }
          break;
        case '/compact':
          if (self.currentAgent) {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Compacting session...', meta: '', tools: [] });
            IABroAPI.post('/api/agents/' + self.currentAgent.id + '/session/compact', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Compaction complete', meta: '', tools: [] });
              self.scrollToBottom();
            }).catch(function(e) { IABroToast.error('Compaction failed: ' + e.message); });
          }
          break;
        case '/stop':
          if (self.currentAgent) {
            IABroAPI.post('/api/agents/' + self.currentAgent.id + '/stop', {}).then(function(res) {
              self.messages.push({ id: ++msgId, role: 'system', text: res.message || 'Run cancelled', meta: '', tools: [] });
              self.sending = false;
              self.scrollToBottom();
            }).catch(function(e) { IABroToast.error('Stop failed: ' + e.message); });
          }
          break;
        case '/usage':
          if (self.currentAgent) {
        ...
        case '/context':
          // Send via WS command
          if (self.currentAgent && IABroAPI.isWsConnected()) {
            IABroAPI.wsSend({ type: 'command', command: 'context', args: '' });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected (' + (IABroAPI.getConnectionState ? IABroAPI.getConnectionState() : 'unknown') + '). Pick an agent or check that your session is still valid.', meta: '', tools: [] });
            self.scrollToBottom();
          }
          break;
        case '/verbose':
          if (self.currentAgent && IABroAPI.isWsConnected()) {
            IABroAPI.wsSend({ type: 'command', command: 'verbose', args: cmdArgs });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected (' + (IABroAPI.getConnectionState ? IABroAPI.getConnectionState() : 'unknown') + '). Pick an agent or check that your session is still valid.', meta: '', tools: [] });
            self.scrollToBottom();
          }
        ...
        case '/queue':
          if (self.currentAgent && IABroAPI.isWsConnected()) {
            IABroAPI.wsSend({ type: 'command', command: 'queue', args: '' });
          } else {
            self.messages.push({ id: ++msgId, role: 'system', text: 'Not connected (' + (IABroAPI.getConnectionState ? IABroAPI.getConnectionState() : 'unknown') + ').', meta: '', tools: [] });
            self.scrollToBottom();
          }
          break;
        case '/status':
          IABroAPI.get('/api/status').then(function(s) {
        ...
        case '/model':
          if (self.currentAgent) {
            if (cmdArgs) {
              IABroAPI.put('/api/agents/' + self.currentAgent.id + '/model', { model: cmdArgs }).then(function(resp) {
                ...
                self.scrollToBottom();
              }).catch(function(e) { IABroToast.error('Model switch failed: ' + e.message); });
            } else {
        ...
        case '/exit':
          ...
          break;
        case '/budget':
          IABroAPI.get('/api/budget').then(function(b) {
          ...
          break;
        case '/peers':
          IABroAPI.get('/api/network/status').then(function(ns) {
          ...
          break;
        case '/a2a':
          IABroAPI.get('/api/a2a/agents').then(function(res) {
    ...
    async loadSession(agentId) {
      var self = this;
      try {
        var data = await IABroAPI.get('/api/agents/' + agentId + '/session');
    ...
    async loadSessions(agentId) {
      try {
        var data = await IABroAPI.get('/api/agents/' + agentId + '/sessions');
    ...
    async createSession() {
      ...
      try {
        await IABroAPI.post('/api/agents/' + this.currentAgent.id + '/sessions', {
        ...
        this.messages = [];
        this.scrollToBottom();
        if (typeof IABroToast !== 'undefined') IABroToast.success(t('chat.session_created'));
      } catch(e) {
        if (typeof IABroToast !== 'undefined') IABroToast.error(t('chat.session_create_failed'));
      }
    },
    ...
    async switchSession(sessionId) {
      if (!this.currentAgent) return;
      try {
        await IABroAPI.post('/api/agents/' + this.currentAgent.id + '/sessions/' + sessionId + '/switch', {});
        this.messages = [];
        await this.loadSession(this.currentAgent.id);
        await this.loadSessions(this.currentAgent.id);
        ...
        if (typeof IABroToast !== 'undefined') IABroToast.error('Failed to switch session');
      }
    },
    ...
    async sendMessage() {
      ...
      if (this.attachments.length) {
        for (var i = 0; i < this.attachments.length; i++) {
          ...
          att.uploading = true;
          try {
            var uploadRes = await IABroAPI.upload(this.currentAgent.id, att.file);
            ...
            IABroToast.error('Failed to upload ' + att.file.name);
    ...
    async _sendPayload(finalText, uploadedFiles, msgImages) {
      ...
      // HTTP fallback
      var t = typeof window.t === 'function' ? window.t : function(s) { return s; };
      if (!IABroAPI.isWsConnected()) {
        IABroToast.info(t('chat.using_http_mode'));
      }
      ...
      try {
        ...
        var res = await IABroAPI.post('/api/agents/' + this.currentAgent.id + '/message', httpBody);
    ...
    stopAgent: function() {
      if (!this.currentAgent) return;
      var self = this;
      IABroAPI.post('/api/agents/' + this.currentAgent.id + '/stop', {}).then(function(res) {
    ...
    killAgent() {
      ...
      IABroToast.confirm(t('chat.stop_agent_title'), t('chat.stop_agent_confirm') + ' "' + name + '"?', async function() {
        try {
          await IABroAPI.del('/api/agents/' + self.currentAgent.id);
          IABroAPI.wsDisconnect();
          self._wsAgent = null;
          self.currentAgent = null;
          self.messages = [];
          try { localStorage.removeItem('of-active-agent'); } catch(e) { /* ignore */ }
          IABroToast.success(t('chat.agent_stopped') + ' "' + name + '"');
          Alpine.store('app').refreshAgents();
        } catch(e) {
          IABroToast.error(t('chat.stop_agent_failed') + ': ' + e.message);
        }
      });
    },

    // Permanently uninstall the agent: kill + remove ~/.iabro/agents/<name>/
    // Issue #1163.
    uninstallAgent: function() {
      ...
      IABroToast.confirm(
        ...
        async function() {
          try {
            var res = await IABroAPI.del('/api/agents/' + agentId + '/uninstall');
            IABroAPI.wsDisconnect();
            self._wsAgent = null;
            self.currentAgent = null;
            self.messages = [];
    ...
    addFiles(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        if (file.size > 10 * 1024 * 1024) {
          IABroToast.warn('File "' + file.name + '" exceeds 10MB limit');
          continue;
        }
        ...
        if (!typeOk) {
          IABroToast.warn('File type not supported: ' + file.name);
          continue;
        }
        var preview = null;
    ...
    startRecording: async function() {
      ...
      try {
        ...
        if (typeof IABroToast !== 'undefined') IABroToast.error('Microphone access denied');
      }
    },
    ...
    _handleRecordingComplete: async function() {
      ...
      try {
        ...
        var upload = await IABroAPI.upload(this.currentAgent.id, file);
        ...
        this.messages = this.messages.filter(function(m) { return !m.thinking || m.role !== 'system'; });
        if (typeof IABroToast !== 'undefined') IABroToast.error('Failed to upload audio: ' + (e.message || 'unknown error'));
      }
    },
