// Vibecodingchile IA,BRO! Analytics Page — Full usage analytics with per-model and per-agent breakdowns
...
function analyticsPage() {
  return {
    ...
    async loadSummary() {
      try {
        this.summary = await IABroAPI.get('/api/usage/summary');
      } catch(e) {
    ...
    async loadByModel() {
      try {
        var data = await IABroAPI.get('/api/usage/by-model');
    ...
    async loadDailyCosts() {
      try {
        var data = await IABroAPI.get('/api/usage/daily');
