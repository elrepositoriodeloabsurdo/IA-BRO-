// Vibecodingchile IA,BRO! Comms Page — Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          IABroAPI.get('/api/comms/topology'),
          IABroAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      if (IABroAPI.apiKey) url += '?token=' + encodeURIComponent(IABroAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await IABroAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await IABroAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        IABroToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        IABroToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await IABroAPI.post('/api/comms/task', body);
        IABroToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        IABroToast.error(e.message || 'Task failed');
      }
