// Vibecodingchile IA,BRO! Setup Wizard — First-run guided setup (Provider + Agent + Channel)
'use strict';
...
function wizardPage() {
  return {
    ...
    async sendTryItMessage(text) {
      ...
      try {
        var res = await IABroAPI.post('/api/agents/' + this.createdAgent.id + '/message', { message: text });
    ...
    async saveKey() {
      ...
      if (!key) {
        IABroToast.error(window.i18n ? window.i18n.t('wizard.enter_api_key') : 'Please enter an API key');
        return;
      }
      this.savingKey = true;
      try {
        await IABroAPI.post('/api/providers/' + encodeURIComponent(provider.id) + '/key', { key: key });
        this.apiKeyInput = '';
        this.keySaved = true;
        this.setupSummary.provider = provider.display_name;
        IABroToast.success((window.i18n ? window.i18n.t('wizard.api_key_saved') : 'API key saved for') + ' ' + provider.display_name);
        await this.loadProviders();
        // Auto-test after saving
        await this.testKey();
      } catch(e) {
        IABroToast.error((window.i18n ? window.i18n.t('wizard.failed_save_key') : 'Failed to save key:') + ' ' + e.message);
      }
    ...
    async testKey() {
      ...
      try {
        ...
        if (result.status === 'ok') {
          IABroToast.success(provider.display_name + ' ' + (window.i18n ? window.i18n.t('wizard.connected') : 'connected') + ' (' + (result.latency_ms || '?') + 'ms)');
        } else {
          IABroToast.error(provider.display_name + ': ' + (result.error || (window.i18n ? window.i18n.t('wizard.connection_failed') : 'Connection failed')));
        }
      } catch(e) {
        this.testResult = { status: 'error', error: e.message };
        IABroToast.error((window.i18n ? window.i18n.t('wizard.test_failed') : 'Test failed:') + ' ' + e.message);
      }
    ...
    async detectClaudeCode() {
      ...
      this.testResult = null;
      try {
        var result = await IABroAPI.post('/api/providers/claude-code/test', {});
        this.testResult = result;
        if (result.status === 'ok') {
          this.claudeCodeDetected = true;
          this.keySaved = true;
          this.setupSummary.provider = 'Claude Code';
          IABroToast.success('Claude Code detected (' + (result.latency_ms || '?') + 'ms)');
        } else {
          this.testResult = { status: 'error', error: 'Claude Code CLI not detected' };
          IABroToast.error('Claude Code CLI not detected. Make sure you\'ve run: npm install -g @anthropic-ai/claude-code && claude auth');
        }
      } catch(e) {
        this.testResult = { status: 'error', error: e.message };
        IABroToast.error('Claude Code CLI not detected. Make sure you\'ve run: npm install -g @anthropic-ai/claude-code && claude auth');
    ...
    async createAgent() {
      ...
      if (!name) {
        IABroToast.error(window.i18n ? window.i18n.t('wizard.enter_agent_name') : 'Please enter a name for your agent');
        return;
      }
      ...
      try {
        ...
        if (res.agent_id) {
          ...
          IABroToast.success((window.i18n ? window.i18n.t('wizard.agent_created') : 'Agent') + ' "' + (res.name || name) + '" ' + (window.i18n ? window.i18n.t('wizard.agent_created_suffix') || 'created' : 'created'));
          await Alpine.store('app').refreshAgents();
        } else {
          IABroToast.error((window.i18n ? window.i18n.t('wizard.failed_create_agent') : 'Failed:') + ' ' + (res.error || 'Unknown error'));
        }
      } catch(e) {
        IABroToast.error((window.i18n ? window.i18n.t('wizard.failed_create_agent') : 'Failed to create agent:') + ' ' + e.message);
      }
    ...
    async configureChannel() {
      ...
      if (!token) {
        IABroToast.error((window.i18n ? window.i18n.t('wizard.enter_token') : 'Please enter the') + ' ' + ch.token_label);
        return;
      }
      ...
      try {
        ...
        fields.token = token;
        await IABroAPI.post('/api/channels/' + ch.name + '/configure', { fields: fields });
        this.channelConfigured = true;
        this.setupSummary.channel = ch.display_name;
        IABroToast.success(ch.display_name + ' ' + (window.i18n ? window.i18n.t('wizard.channel_configured') : 'configured and activated.'));
      } catch(e) {
        IABroToast.error((window.i18n ? window.i18n.t('wizard.failed_configure') : 'Failed:') + ' ' + (e.message || 'Unknown error'));
      }
    ...
    finish() {
      localStorage.setItem('iabro-onboarded', 'true');
