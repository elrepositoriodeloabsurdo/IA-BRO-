// Vibecodingchile IA,BRO! Skills Page — OpenClaw/ClawHub ecosystem + local skills + MCP servers
'use strict';
...
function skillsPage() {
  return {
    ...
    async loadSkills() {
      ...
      this.loadError = '';
      try {
        var data = await IABroAPI.get('/api/skills');
    ...
    async openSkillConfig(skill) {
      ...
      try {
        var data = await IABroAPI.get('/api/skills/' + encodeURIComponent(skill.name) + '/config');
    ...
    async saveSkillConfig() {
      ...
      if (this.hasInvalidConfig()) {
        IABroToast.error('Fill in all required variables before saving.');
        return;
      }
      ...
      try {
        await IABroAPI.put('/api/skills/' + encodeURIComponent(this.configSkill.name) + '/config', { values: payload });
        IABroToast.success('Saved, reloading agents\u2026');
        ...
        IABroToast.error('Save failed: ' + (e.message || 'unknown error'));
      }
    ...
    async resetSkillConfigVar(name) {
      ...
      try {
        await IABroAPI.del('/api/skills/' + encodeURIComponent(this.configSkill.name) + '/config/' + encodeURIComponent(name));
        IABroToast.success('Reset ' + name);
        // Refresh modal state from server.
        var data = await IABroAPI.get('/api/skills/' + encodeURIComponent(this.configSkill.name) + '/config');
        this.configResolved = data.resolved || {};
        this.configDraft[name] = '';
      } catch(e) {
        var msg = e.message || 'Reset failed';
        if (msg.indexOf('required') !== -1 || msg.indexOf('409') !== -1) {
          IABroToast.error('Cannot reset: ' + decl.description + ' is required with no fallback.');
        } else {
          IABroToast.error('Reset failed: ' + msg);
        }
      }
    },
    ...
    async searchClawHub() {
      ...
      this.clawhubError = '';
      try {
        var data = await IABroAPI.get('/api/clawhub/search?q=' + encodeURIComponent(this.clawhubSearch.trim()) + '&limit=20');
    ...
    async browseClawHub(sort) {
      ...
      try {
        var data = await IABroAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20');
    ...
    async loadMoreClawHub() {
      ...
      this.clawhubLoading = true;
      try {
        var data = await IABroAPI.get('/api/clawhub/browse?sort=' + this.clawhubSort + '&limit=20&cursor=' + encodeURIComponent(this.clawhubNextCursor));
    ...
    async showSkillDetail(slug) {
      ...
      try {
        var data = await IABroAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug));
        this.skillDetail = data;
      } catch(e) {
        IABroToast.error('Failed to load skill details');
      }
    ...
    async viewSkillCode(slug) {
      ...
      try {
        var data = await IABroAPI.get('/api/clawhub/skill/' + encodeURIComponent(slug) + '/code');
        this.skillCode = data.code || '';
        this.skillCodeFilename = data.filename || 'source';
        this.showSkillCode = true;
      } catch(e) {
        IABroToast.error('Could not load skill source code');
      }
    ...
    async installFromClawHub(slug) {
      ...
      this.installResult = null;
      try {
        var data = await IABroAPI.post('/api/clawhub/install', { slug: slug });
        this.installResult = data;
        if (data.warnings && data.warnings.length > 0) {
          IABroToast.success('Skill "' + data.name + '" installed with ' + data.warnings.length + ' warning(s)');
        } else {
          IABroToast.success('Skill "' + data.name + '" installed successfully');
        }
        ...
        if (msg.includes('already_installed')) {
          IABroToast.error('Skill is already installed');
        } else if (msg.includes('SecurityBlocked')) {
          IABroToast.error('Skill blocked by security scan');
        } else {
          IABroToast.error('Install failed: ' + msg);
        }
      }
    ...
    uninstallSkill: function(name) {
      ...
      IABroToast.confirm(
        ...
        async function() {
          try {
            await IABroAPI.post('/api/skills/uninstall', { name: name });
            IABroToast.success('Skill "' + name + '" uninstalled');
            await self.loadSkills();
          } catch(e) {
            IABroToast.error('Failed to uninstall skill: ' + e.message);
          }
        }
      );
    },
    ...
    async createDemoSkill(skill) {
      try {
        await IABroAPI.post('/api/skills/create', {
          name: skill.name,
          description: skill.description,
          runtime: 'prompt_only',
          prompt_context: skill.prompt_context || skill.description
        });
        IABroToast.success('Skill "' + skill.name + '" created');
        this.tab = 'installed';
        await this.loadSkills();
      } catch(e) {
        IABroToast.error('Failed to create skill: ' + e.message);
      }
    },
    ...
    async loadMcpServers() {
      ...
      try {
        var data = await IABroAPI.get('/api/mcp/servers');
