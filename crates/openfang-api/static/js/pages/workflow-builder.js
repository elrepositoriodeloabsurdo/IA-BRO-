// Vibecodingchile IA,BRO! Visual Workflow Builder — Drag-and-drop workflow designer
'use strict';
...
function workflowBuilder() {
  return {
    ...
    async init() {
      ...
      try {
        var list = await IABroAPI.get('/api/agents');
    ...
    async saveWorkflow() {
      ...
      try {
        await IABroAPI.post('/api/workflows', {
          name: this.workflowName || 'untitled',
          description: this.workflowDescription || '',
          steps: steps
        });
        IABroToast.success('Workflow saved!');
