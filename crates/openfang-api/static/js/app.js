// IA,BRO! App — Alpine.js init, hash router, global store
'use strict';
...
document.addEventListener('alpine:init', function() {
  ...
  var savedKey = localStorage.getItem('iabro-api-key');
  if (savedKey) IABroAPI.setAuthToken(savedKey);
  ...
  Alpine.store('app', {
    ...
    pendingAgent: null,
    focusMode: localStorage.getItem('iabro-focus') === 'true',
    ...
    toggleFocusMode() {
      ...
      localStorage.setItem('iabro-focus', this.focusMode);
    },

    async refreshAgents() {
      try {
        var agents = await IABroAPI.get('/api/agents');
    ...
    async refreshApprovals() {
      try {
        var data = await IABroAPI.get('/api/approvals');
        ...
        var signature = pending
          ...
          .sort()
          .join(',');
        if (pending.length > 0 && signature !== this.lastPendingApprovalSignature && typeof IABroToast !== 'undefined') {
          IABroToast.warn('An agent is waiting for approval. Open Approvals to review.');
        }
    ...
    async checkStatus() {
      try {
        var s = await IABroAPI.get('/api/status');
        this.connected = true;
        this.booting = false;
        this.lastError = '';
        ...
        console.warn('[IA,BRO!] Status check failed:', e.message);
      }
    },

    async checkOnboarding() {
      if (localStorage.getItem('iabro-onboarded')) return;
      try {
        var config = await IABroAPI.get('/api/config');
    ...
    async checkAuth() {
      try {
        ...
        var authInfo = await IABroAPI.get('/api/auth/check');
      ...
      try {
        ...
        if (e.message && (e.message.indexOf('Not authorized') >= 0 || e.message.indexOf('401') >= 0 || e.message.indexOf('Missing Authorization') >= 0 || e.message.indexOf('Unauthorized') >= 0)) {
          var saved = localStorage.getItem('iabro-api-key');
          if (saved) {
            IABroAPI.setAuthToken('');
            localStorage.removeItem('iabro-api-key');
          }
          this.showAuthPrompt = true;
        }
      }
    ...
    submitApiKey(key) {
      ...
      IABroAPI.setAuthToken(key.trim());
      localStorage.setItem('iabro-api-key', key.trim());
      this.showAuthPrompt = false;
      this.refreshAgents();
    },
    ...
    async sessionLogin(username, password) {
      try {
        var result = await IABroAPI.post('/api/auth/login', { username: username, password: password });
        if (result.status === 'ok') {
          this.sessionUser = result.username;
          this.showAuthPrompt = false;
          this.refreshAgents();
        } else {
          IABroToast.error(result.error || 'Login failed');
        }
      } catch(e) {
        IABroToast.error(e.message || 'Login failed');
      }
    },

    async sessionLogout() {
      try {
        await IABroAPI.post('/api/auth/logout');
      } catch(e) { /* ignore */ }
      this.sessionUser = null;
      this.showAuthPrompt = true;
    },
...
function app() {
  return {
    page: 'agents',
    themeMode: localStorage.getItem('iabro-theme-mode') || 'system',
    theme: (() => {
      var mode = localStorage.getItem('iabro-theme-mode') || 'system';
      if (mode === 'system') return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
      return mode;
    })(),
    sidebarCollapsed: localStorage.getItem('iabro-sidebar') === 'collapsed',
    ...
    init() {
      ...
      IABroAPI.onConnectionChange(function(state) {
    ...
    setTheme(mode) {
      this.themeMode = mode;
      localStorage.setItem('iabro-theme-mode', mode);
      if (mode === 'system') {
    ...
    toggleSidebar() {
      ...
      localStorage.setItem('iabro-sidebar', this.sidebarCollapsed ? 'collapsed' : 'expanded');
    },
    ...
    async pollStatus() {
      ...
      this.wsConnected = IABroAPI.isWsConnected();
    }
  };
}
