// IA,BRO! API Client — Fetch wrapper, WebSocket manager, auth injection, toast notifications
'use strict';

// ── Toast Notification System ──
var IABroToast = (function() {
  var _container = null;
  var _toastId = 0;
...
function friendlyError(status, serverMsg) {
  if (status === 0 || !status) return 'Cannot reach daemon — is iabro running?';
  ...
  function request(method, path, body) {
    ...
    return fetch(BASE + path, opts).then(function(r) {
      ...
      if (e.name === 'TypeError' && e.message.includes('Failed to fetch')) {
        ...
        throw new Error('Cannot connect to daemon — is iabro running?');
      }
      throw e;
    });
  }
    ...
    try {
      ...
      socket.onopen = function() {
        ...
        if (_reconnectAttempt > 0) {
          IABroToast.success('Reconnected');
          _reconnectAttempt = 0;
        }
      ...
      socket.onclose = function(e) {
        ...
        if (_wsAgentId && _reconnectAttempts < MAX_RECONNECT && e.code !== 1000) {
          ...
          if (_reconnectAttempts === 1) {
            IABroToast.warn('Connection lost, reconnecting...');
          }
        ...
        if (_wsAgentId && _reconnectAttempts >= MAX_RECONNECT) {
          setConnectionState('disconnected');
          IABroToast.error('Connection lost — switched to HTTP mode', 0);
        }
